...
4. **SSH / Remote Agents** — A built-in SSH terminal client ([SwiftTerm](https://github.com/migueldeicaza/SwiftTerm) MIT + [swift-nio-ssh](https://github.com/apple/swift-nio-ssh) Apache-2.0, pure Swift) for any host, plus a **Projects** tab that drives a CLI coding **agent** on a Mac over SSH — tmux-backed for persistence, with a gaze/dictation composer + quick-key row. Three agents are switchable per host (`SSHAgent`: `.claude` / `.copilot` / `.custom`), the choice remembered as the host's default: **Claude Code** (`claude`, `CLAUDE_CODE_OAUTH_TOKEN`), **GitHub Copilot CLI** (`copilot`, `COPILOT_GITHUB_TOKEN`), or a free-form **Custom** command + env-var name. The device identity is a Secure Enclave P-256 key; only its public key is installed on the host. Because the macOS Keychain is unreachable over SSH, each agent auths via a long-lived token stored **per-agent** in the Vision Pro keychain and injected inline per session — no `sshd_config` edit. Claude's token comes from `claude setup-token`; Copilot's is minted by an **in-app GitHub OAuth device flow** (`GitHubDeviceFlow`, public client `Ov23ctDVkRmgkPke0Mmm`) — the headset runs the device-authorization flow itself and captures the `access_token`, so Copilot's own keychain (which it accesses via the native `keytar` module, unreadable over SSH) is never relied on. The companion macOS app also gained **text-only keyboard injection** (CGEvent Unicode + backspace, no modifiers — DuckyScript-safe) so the VNC soft keyboard can route typing through the Mac when a companion is linked.
...
- **macOS Keychain is unreachable over SSH** (it's bound to the GUI login session). So a CLI agent can't read its stored OAuth login in an SSH session. Fix: a long-lived token stored in the Vision Pro keychain (`KeychainStore`) **per connection AND per agent** and injected per session as the agent's env var (Claude reads `CLAUDE_CODE_OAUTH_TOKEN`, Copilot reads `COPILOT_GITHUB_TOKEN` — both ahead of any stored credential). Keychain account scheme: Claude keeps the bare connection UUID (so tokens stored before multi-agent support survive untouched), Copilot/Custom are suffixed `.<agent>` (`SavedConnection.tokenAccount`). The per-agent `sshHasAuthToken`/`sshHasCopilotToken`/`sshHasCustomToken` bools are UI-only flags; the secret is always the keychain.
- **Copilot tokens are minted in-app, not pasted.** `GitHubDeviceFlow` runs GitHub's OAuth device-authorization flow from the headset (Copilot CLI is a public GitHub App client — no secret): `POST /login/device/code` → show `user_code` + `verification_uri` natively → poll `POST /login/oauth/access_token` (honoring `authorization_pending`/`slow_down`) → store the `access_token`. Confirmed against the installed `@github/copilot`: client_id `Ov23ctDVkRmgkPke0Mmm`, scope `read:user,read:org,repo,gist`. Copilot accepts this as "an OAuth token from the GitHub Copilot CLI app." Copilot stores its own creds via the native `keytar` addon (Security.framework, not the `security` CLI) → unreadable over SSH, which is why we mint+inject rather than let it persist. A pasted fine-grained PAT (with the "Copilot Requests" permission) is supported as a fallback; classic `ghp_` tokens are not.
